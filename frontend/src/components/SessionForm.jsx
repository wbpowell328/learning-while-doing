...
const ROWS_1D = [
  ...
  { kind: 'int', label: 'Lookahead horizon for knowledge gradient (ρˡᵏʰᵈ)',
    ...
    range: '[1–100]',
    desc: 'If =1, the KG computes the value of a single experiment for the decision we make now. If >1, the KG assumes we perform ρˡᵏʰᵈ replications, reducing the noise of the observation. Used by all KG-family policies (including Ryzhov).' },
...
const ROWS_2D = [
  ...
  { kind: 'int', label: 'Lookahead horizon for knowledge gradient (ρˡᵏʰᵈ)',
    source: 'mStarStr', min: 1, max: 100,
    range: '[1–100]',
    desc: 'If =1, the KG computes the value of a single experiment. If >1, the KG assumes we perform ρˡᵏʰᵈ replications, reducing the noise of the observation. Used by all KG-family policies (including Ryzhov).' },
  { kind: 'number', label: 'IE exploration coefficient (ρᴵᴱ)',
    source: 'zAlphaStr', min: 0, step: 'any',
    range: '[0 – 3]',
    desc: 'IE score = μ + ρᴵᴱ·σ. 0 = pure exploitation (greedy); larger explores more. Used only by the IE policy.' },
