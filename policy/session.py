...
regardless of which impparam was proposed.  This means cost differences across
steps reflect only impparam changes, not different noise draws — a variance
...
class SessionConfig:
    ...
    best_impparam_grid: int = 200  # grid resolution for best_impparam()
...
class Session:
    ...
    Typical usage::
        ...
        for _ in range(n_experiments):
            result = session.step()
        optimal_c = session.best_impparam()
    """
    ...
    def __init__(
        ...
        self._history: list[tuple[float, float]] = []   # (impparam, cost)
    ...
    def evaluate(self, impparam: float) -> SimResult:
        """
        ...
        result = simulate(
            config=self._sim_config,
            impparam=float(impparam),
            horizon_weeks=self._sc.horizon_weeks,
            session_seed=self._session_seed,
            experiment_index=self._step_count,
        )
        self._belief.update(float(impparam), result.total_cost)
        self._history.append((float(impparam), result.total_cost))
        self._step_count += 1
        return result

    def observe(self, impparam: float, total_cost: float) -> None:
        """
        ...
        """
        self._belief.update(float(impparam), float(total_cost))
        self._history.append((float(impparam), float(total_cost)))

    def best_impparam(self) -> float:
        """
        ...
        if self._belief.n_observations == 0:
            ...
            return (cfg.impparam_min + cfg.impparam_max) / 2.0

        cfg = self._acq_config
        grid = np.linspace(cfg.impparam_min, cfg.impparam_max, self._sc.best_impparam_grid)
    ...
    def history(self) -> list[tuple[float, float]]:
        """Ordered (impparam, cost) pairs from both step() and observe() calls."""
