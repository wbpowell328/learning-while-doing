...
Two distinct kinds of tunable parameter appear throughout this document.
They are given deliberately different symbols to avoid confusion.
...
\begin{tabular}{@{}p{2.5cm}p{3.5cm}p{9.5cm}@{}}
  ...
  \midrule
  $\theta$ & \code{impparam} &
    The \emph{implementation parameter}. The scalar (or, eventually, vector)
    that governs the deployed system's behavior — for the fund example,
    the cash-buffer ratio previously written $C^{*}$. This is what the
    learner is trying to tune. \\[3pt]
  $\rho$   & \code{learnparam} &
    A tunable parameter of the \emph{learning policy} itself — e.g.\
    the exploration weight of an IE (LCB) rule. Currently only IE has
    such a parameter; a future modified KG will as well. \\
  \bottomrule
\end{tabular}
\end{center}

Wherever a symbol appears alone in the text without qualification, this
...
\[
  \tau_t \;=\; \max\!\bigl(\eta \cdot g_t,\; -\code{Inv}_{t+1}^{-}\bigr),
\]
where $\eta \in (0,1]$ is the rebalance speed (default $1$, i.e.\
...
\begin{equation}
  \code{IE}(x;\, \rho) \;=\; \muN(x) \;-\; \rho \cdot \sigN(x),
  \label{eq:ie-lcb}
\end{equation}
where $\rho$ is the learning-policy parameter (Notation conventions,
p.~\pageref{sec:overview}). The policy picks
$\theta_{n+1} = \arg\min_{x \in \mathcal{G}} \code{IE}(x;\rho)$.
$\rho \geq 0$ controls the exploration--exploitation tradeoff:
\begin{itemize}[nosep]
  \item $\rho = 0$: pure exploitation ($\arg\min \muN$).
  \item $\rho \approx 1.96$: 95\% lower confidence bound.
  \item $\rho \to \infty$: pure exploration ($\arg\max \sigN$, i.e.\
    maximum-entropy reduction).
\end{itemize}
The IE batch benchmark evaluates 21 values of $\rho \in \{0, 0.2, 0.4,
\dots, 4.0\}$. In the code, $\rho$ is currently stored on the
\code{AcquisitionConfig} as the field \code{z\_alpha} (retaining its
semantic name from the classical confidence-bound literature); future
learning policies that also carry a tunable parameter will expose it
under the more generic name \code{learnparam}.
...
\begin{itemize}[nosep]
  ...
  \item \textbf{IE family (21 policies):} \code{IEPolicy} at $\rho \in
    \{0, 0.2, 0.4, \dots, 4.0\}$.
\end{itemize}
...
\begin{tabular}{@{}p{4.2cm}p{2cm}p{8.5cm}@{}}
  ...
  $\eta$ (\code{rebalance\_speed})                 & $1.0$    & Fraction of gap closed each end-of-day rebalance. \\
  $[\theta_{\min}, \theta_{\max}]$ (\code{impparam\_min}, \code{impparam\_max}) & $[0.01, 0.20]$ & Legal range of $\theta$. \\
...
\begin{tabular}{@{}p{4.2cm}p{2cm}p{8.5cm}@{}}
  ...
  Symbol / field & Default & Description \\
  \midrule
  $\theta_{\min}, \theta_{\max}$ (\code{impparam\_min}, \code{impparam\_max}) & $0.01, 0.20$ & Acquisition-grid endpoints (mirror \code{SimConfig}). \\
  $M$ (\code{grid\_size})       & $100$        & Number of grid points $\mathcal{G}$. \\
  $\rho$ (\code{z\_alpha})      & $0.0$        & Exploration weight in IE — the learning-policy parameter (see Eq.~\eqref{eq:ie-lcb}). \\
  \bottomrule
\end{tabular}
