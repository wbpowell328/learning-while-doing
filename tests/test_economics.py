...
Estimating F(C*) by averaging many seeds should yield:
  - high cost at very low C* (shortfall-dominated)
  - high cost at very high C* (drag-dominated)
...
def test_u_shape():
    ...
    assert cost_low > cost_mid, (
        f"Expected cost at C*=0.01 ({cost_low:.2f}) > cost at C*=0.08 ({cost_mid:.2f}); "
        "shortfall cost should dominate at very low C*"
    )
    assert cost_high > cost_mid, (
        f"Expected cost at C*=0.20 ({cost_high:.2f}) > cost at C*=0.08 ({cost_mid:.2f}); "
        "opportunity cost (cash drag) should dominate at very high C*"
    )


def test_low_c_star_has_shortfall():
    """At very low C*, shortfall cost should be non-trivial (not zero)."""
    results = [
    ...
    assert mean_shortfall > 0, "Expected non-zero shortfall at C*=0.01"


def test_high_c_star_has_drag():
    """At very high C*, opportunity cost should be non-trivial."""
    results = [
    ...
    assert mean_drag > 0, "Expected non-zero opportunity cost at C*=0.20"
