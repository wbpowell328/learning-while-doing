...
def test_step_c_star_in_bounds():
    ...
    for _ in range(5):
        ...
        assert ACQ_MIN <= data["c_star"] <= ACQ_MAX
...
# ---------------------------------------------------------------------------
# 6. best_c_star in bounds
# ---------------------------------------------------------------------------

def test_step_best_c_star_in_bounds():
    sid = make_session()
    for _ in range(3):
        data = client.post(f"/sessions/{sid}/step").json()
