...
def test_same_inputs_identical_result():
    ...
    r1 = simulate(CFG, impparam=0.05, horizon_weeks=4, session_seed=42, experiment_index=0)
    r2 = simulate(CFG, impparam=0.05, horizon_weeks=4, session_seed=42, experiment_index=0)
...
def test_different_experiment_index_different_cost():
    ...
    costs = {
        simulate(CFG, impparam=0.05, horizon_weeks=52, session_seed=42, experiment_index=i).total_cost
        for i in range(20)
    }
...
def test_different_seed_different_result():
    ...
    r1 = simulate(CFG, impparam=0.05, horizon_weeks=4, session_seed=1, experiment_index=0)
    r2 = simulate(CFG, impparam=0.05, horizon_weeks=4, session_seed=2, experiment_index=0)
